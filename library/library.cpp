...
int checkout(int bookid, int patronid){
	...
	if (patrons[patron_num].number_books_checked_out == MAX_BOOKS_ALLOWED_OUT){
		return TOO_MANY_OUT;
	}
	patrons[patron_num].number_books_checked_out++;
	books[book_num].loaned_to_patron_id = patronid;
	books[book_num].state = OUT;
	saveBooks(books,BOOKFILE.c_str());
	savePatrons(patrons, PATRONFILE.c_str());
	return SUCCESS;
}
